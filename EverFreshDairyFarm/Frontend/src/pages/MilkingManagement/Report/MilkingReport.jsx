...
function MilkingReport() {
    ...
    const downloadPDF = () => {
        ...
        // Add report title
        doc.setFontSize(18);
        doc.text("Milking Management Report", doc.internal.pageSize.getWidth()/2, 35, { align: 'center' });
        ...
        const totalMilk = milkingData.reduce((sum, item) => sum + parseFloat(item.amountofMilk), 0);
        const passCount = milkingData.filter(item => item.qualityCheckResult === "pass").length;
        doc.text(`Total Milk Collected: ${totalMilk.toFixed(2)} L`, 80, 57);
        doc.text(`Quality Pass Rate: ${((passCount/milkingData.length) * 100).toFixed(1)}%`, 150, 57);
        ...
        milkingData.forEach((item, index) => {
            ...
            if (yPosition > doc.internal.pageSize.getHeight() - 20) {
                ...
                doc.setTextColor(255, 255, 255);
                doc.setFontSize(12);
                doc.text("Milking Management Report (Continued)", doc.internal.pageSize.getWidth()/2, 15, { align: 'center' });
        ...
        yPosition += 7;
        doc.text(`Quality Pass Rate: ${((passCount/milkingData.length) * 100).toFixed(1)}%`, 20, yPosition);
        yPosition += 7;
