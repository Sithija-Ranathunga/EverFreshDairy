...
export const createMilkingData = async (req, res) => {
  try {
    ...
    if (milkAmount < 5 || temperature < 38) {
      const alertMessage =
        milkAmount < 5
          ? `Cow ${newmilkingData.cowId} has low milk production (${newmilkingData.amountofMilk}L)`
          : `Cow ${newmilkingData.cowId} has abnormal temperature (${newmilkingData.temperature}°C)`;
...
export const updateMilkingData = async (req, res) => {
  try {
    const updatemilkingDataRecord = await MilkingDatamodel.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
    if (!updatemilkingDataRecord)
      return res.status(404).json({ message: "Record not found" });
...
export const deleteMilkingData = async (req, res) => {
  try {
    const deletemilkingDataRecords = await MilkingDatamodel.findByIdAndDelete(
      req.params.id
    );
    if (!deletemilkingDataRecords)
      return res.status(404).json({ message: "Record not found" });
